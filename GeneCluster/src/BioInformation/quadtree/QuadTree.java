...
import BioInformation.Util.RandomNumberUtil;
...
public class QuadTree {
	...
	public void set(double x, double y, Object value) {
		...
		if (this.find(root, x, y) != null) {// if there conatains same point,add
											// random number
			x += RandomNumberUtil.rd.nextDouble() / 10000000;
			y += RandomNumberUtil.rd.nextDouble() / 10000000;
		}
	...
	public Square searchWithin(final double xmin, final double ymin, final double xmax, final double ymax) {
		...
		this.navigate(this.root_, new Func() {
			...
			public void call(QuadTree quadTree, Node node) {
				Point pt = node.getPoint();
				if (pt.getX() > xmin && pt.getX() < xmax && pt.getY() > ymin && pt.getY() < ymax) {
					arr.add(Integer.parseInt(node.getPoint().getValue().toString()));
				}
			}
		}, xmin, ymin, xmax, ymax);
		return new Square(xmin,ymin,xmax,ymax,arr);
	}
	/**
	 * Returns all the ids within the range
	 * 
	 * @param xmin
	 * @param ymin
	 * @param xmax
	 ...
	 * @return
	 */
	/*public Set<Integer> getIdsWithin(final double xmin, final double ymin, final double xmax, final double ymax) {
		Map<String, Grid> gridMap = GridSingleton.getInstance().getGridMap();
		Point[] points = searchWithin(xmin, ymin, xmax, ymax);
		Set<Integer> ps = new HashSet<Integer>();
		for (Point point : points) {
			System.out.println("x,y" + point.getX() + "," + point.getY());
			Set<Integer> ids = gridMap.get(find(getRootNode(), point.getX(), point.getY()).getGridId()).getIds();
			ps.addAll(ids);
		}
		return ps;
	}*/
	/*
	 * public int searchWithin(final double xmin, final double ymin, final
	 * double xmax, final double ymax) { this.navigate(this.root_, new Func() {
	 * 
	 * @Override public void call(QuadTree quadTree, Node node) { Point pt =
	 * node.getPoint(); if (pt.getX() >= xmin && pt.getX() <= xmax && pt.getY()
	 * >= ymin && pt.getY() <= ymax) { numOfPoints +=
	 * Double.parseDouble(String.valueOf(node.getPoint().getValue()));
	 * System.out.println("numOfPoints:"+numOfPoints); } } }, xmin, ymin, xmax,
	 * ymax); int ret = (int)numOfPoints; numOfPoints =0; return ret; }
	 */
	...
	/**
	 * Put a new id of point and according gridID into hashmap
	 * 
	 * @param gridId
	 * @param id
	 */
	/*private void put(String gridId, Object id) {
		Map<String, Grid> gridMap = GridSingleton.getInstance().getGridMap();
		Grid grid = null;
		...
		grid.add(Integer.parseInt(id.toString()));
	...
	private boolean insert(Node parent, Point point) {
		...
		//put(parent.getGridId(), point.getValue());
		...
		case LEAF:
			...
			break;
		case POINTER:
			Node child = this.getQuadrantForPoint(parent, point.getX(), point.getY());
			//put(child.getGridId(), point.getValue());
		
			result = this.insert(child, point);
			break;
